...
const MediaPage = () => {
  ...
  const loadPreLoadData = async (page = 1) => {
    ...
    const response = await getRequestApi(`post/type/media/${language}`, query);
    if (response.status) {
  ...
  const openModal = (index) => {
    ...
    const selectedImage = loadData[index]?.featured_image?.url;
    const modalImage = document.getElementById("modalImage");
    const modalCaption = document.getElementById("modalCaption");

    if (modalImage && modalCaption && selectedImage) {
