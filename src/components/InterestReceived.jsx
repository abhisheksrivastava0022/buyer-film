...
const InterestReceived = () => {
    ...
    const ApprovedBuyer = async (data) => {
        const response = await postRequestApi("film/buyer-approved", data);
        if (response.status) {
            // alert("connected requested")
            loadPreLoadData();
        }
    }
    const ApprovedSeller = async (data) => {
        const response = await postRequestApi("film/seller-approved", data);
        if (response.status) {
            // alert("connected requested")
            loadPreLoadData();
        }
    ...
    const loadPreLoadData = async (page = 1) => {
        try {
            const data = await getRequestApi('film/buyer-interest-received');
            if (data.status) {
                setData(data.data);
                setDatatocheck(data.datatocheck);

            }
            const data1 = await getRequestApi('film/seller-interest-received');
            if (data1.status) {
                setData1(data1.data);
        ...
        <>
            ...
            <main className="col-md-9 ms-sm-auto col-lg-10 px-md-4 main-content">
                ...
                <div className=" main-content-space ">
                    <div className='pagetitle-name'><h1>Interest Received</h1>
                    </div>
                                                                    ...
                                                                    <ul className="col-md-12 col-sm-12" style={{ textAlign: "right" }}>
                                                                        ...
                                                                        <li>
                                                                            <button
                                                                                ...
                                                                                type="submit"
                                                                                onClick={() =>
                                                                                    ApprovedSeller({ id: row.id, status: 1, type: 4 })
                                                                                }
                                                                            >
                                                                                Approved
                                                                            </button>
