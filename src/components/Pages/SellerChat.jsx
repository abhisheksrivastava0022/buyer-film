...
const SellerChat = () => {
    ...
    const fetchChatDetails = async () => {
        ...
        const response = await getRequestApi(`film/chat/${id}/3`);
    ...
    useEffect(() => {
        ...
        setInterval(async () => {
            const response = await getRequestApi(`film/find-new-chat/${id}/3`);
            if (response.data) {
                fetchChatDetails();
            }
    ...
    const handleSubmit = async (e) => {
        if (message) {
            const response = await postRequestApi(`film/chat/${id}/3`, { message });
            if (response.status) {
                // alert("data pass")
                fetchChatDetails();
            }
                                        ...
                                        {chatlist.map((row) => (
                                            row.receiver_id == id ? (
                                                ...
                                                <li key={row.id} className="left clearfix">
                                                    <span className="chat-img pull-left">
                                                        <img
                                                            src={buyer?.profile_img?.url ? `${dataurl}${process.env.REACT_APP_BASE_PREFIX}/file/${buyer.profile_img.url}` : defaultimg}
                                                            alt={buyer?.profile_img?.name ? `${buyer.profile_img.name}` : "defaultimg"} />
                                                    </span>
                                                    <div className="chat-body clearfix">
                                                        <p>{row.message}</p>
                                                        <div className="header">
